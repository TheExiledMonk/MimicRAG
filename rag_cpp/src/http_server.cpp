...
void Handle(int socket, RagEngine& engine, std::shared_mutex& engine_gate) {
    ...
    try {
        ...
        else if (method == "POST" && path == "/v1/evidence") Reply(socket, 200, engine.EvidenceAppend(input));
        else if (method == "POST" && path == "/v1/evidence/inspect") Reply(socket, 200, engine.EvidenceInspect(input));
        ...
        else if (method == "POST" && path == "/v1/memory/reject") Reply(socket, 200, engine.MemoryReject(input));
        else if (method == "POST" && path == "/v1/memory/dispute") Reply(socket, 200, engine.MemoryDispute(input));
        else if (method == "POST" && path == "/v1/memory/due") Reply(socket, 200, engine.MemoryDue(input));
