...
The native `mimicrag_server` can be built with a fail-closed startup license check. It is disabled by default. On Linux it uses `/etc/machine-id`; on macOS it uses the system `kern.uuid`:
...
cmake --build build-licensed --target mimicrag_server
build-licensed/rag_cpp/mimicrag_server --print-license-id
```
...
nohup build-licensed/rag_cpp/mimicrag_server --config /etc/mimicdb/mimicrag.json \
