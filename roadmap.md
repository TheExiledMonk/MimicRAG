...
- [x] Add explicit namespaces for working, episodic, semantic, procedural, preference,
  prospective, and negative memory.
- [x] Store an immutable event/evidence log for conversations, tool results, corrections, and task
  outcomes before forming durable memories.
- [x] Define a versioned memory record containing subject, canonical statement, type, event time,
  validity interval, confidence, importance, provenance, reinforcement, decay policy, visibility,
  and lifecycle status.
- [x] Link every inferred memory to supporting evidence and optional contradicting evidence.
- [x] Keep raw evidence authoritative and make every consolidation reversible.
- [x] Never allow imported documents or inferred memories to modify agent identity or policy.

### Short-term and working memory

- [x] Add a compact mutable working-memory state for the current objective, active entities,
  constraints, assumptions, unresolved questions, recent observations, and pending operations.
- [x] Add session and task boundaries with configurable expiration.
- [x] Promote only selected outcomes from working memory into durable memory.
- [x] Build bounded context packets instead of replaying complete conversation histories.
- [x] Persist crash-safe task state without making temporary observations permanently prominent.

### Long-term memory and associations

- [x] Add typed, weighted, timestamped graph relations such as `supports`, `contradicts`,
  `supersedes`, `caused`, `resolved_by`, `preference_of`, and `remind_when`.
- [x] Add temporal reasoning across event time, recording time, validity, and supersession.
- [x] Reinforce memories through repeated independent evidence and successful use.
- [x] Decay retrieval priority for unused episodic memories without immediately deleting evidence.
- [x] Preserve superseded memories for historical questions while excluding them from current-fact
  retrieval.
- [x] Add prospective memories activated by bounded time, entity, state, or action conditions.

### Explicit memory API

- [x] Add native `remember`, `recall`, `correct`, `forget`, `inspect`, and `export` operations.
- [x] Require tenant, owner, visibility, sensitivity, and allowed-purpose metadata.
- [x] Add confirmation controls for identity-level, sensitive, legal, financial, and personal
  memories.
- [x] Make explicit user corrections outrank inferred memories.
- [x] Implement verifiable forgetting across source events where allowed, derived memories,
  embeddings, lexical indexes, graph edges, caches, traces, backups, and compaction.
- [x] Add an audit view explaining why a memory was formed, changed, recalled, or suppressed.

### Remote LLM memory manager

- [x] Add a separate provider-independent `memory_model` configuration that can reuse the existing
  remote chat connection, use a cheaper specialist model, or run locally.
- [x] Default memory extraction and consolidation to the provider's lowest reliable temperature,
  normally `0.0`; leave `top_p` unset unless provider-specific evaluation proves a benefit.
- [x] Require strict versioned structured output for every proposed memory operation.
- [x] Require existing evidence IDs and verify quoted supporting spans before accepting proposals.
- [x] Validate types, confidence bounds, authorization, provenance, relationships, and operation
  limits in native code.
- [x] Allow the model to propose creates, reinforcements, associations, supersessions, disputes,
  reminders, and deletions; never allow it to mutate storage directly.
- [x] Quarantine malformed, unsupported, low-confidence, contradictory, or sensitive proposals for
  rejection or confirmation.
- [x] Record provider, model identity, prompt/schema version, input evidence IDs, output, latency,
  token usage, validation result, and final mutation IDs.
- [x] Keep explicit memory operations and retrieval synchronous; run extraction, consolidation,
  reflection, association discovery, and decay asynchronously.
- [x] Ensure remote-model failure never blocks ordinary RAG retrieval or agent responses.

### Memory formation and consolidation

- [x] Extract candidate facts, preferences, decisions, commitments, corrections, procedures,
  failures, relationships, and unresolved work at task or session boundaries.
- [x] Deduplicate candidates against active and superseded memories before writing.
- [x] Consolidate repeated episodes into semantic or procedural memory while retaining evidence.
- [x] Distinguish true updates from contextual differences and unresolved contradictions.
- [x] Add bounded reflection jobs that propose lessons, changed facts, reusable procedures, and
  expired working state.
- [x] Cache processing by evidence hash, model identity, prompt version, and schema version.
- [x] Set per-session limits for remote calls, tokens, accepted memories, associations, and
  reflection depth.

### Context-dependent recall

- [x] Rank memory using semantic and lexical relevance, confidence, importance, reinforcement,
  temporal validity, decay, trust, and current-goal relevance.
- [x] Select different memory mixes for coding, planning, conversation, research, and sensitive
  actions.
- [x] Return compact sections for current state, durable facts, relevant episodes, applicable
  procedures, preferences, commitments, uncertainty, and provenance.
- [x] Support combined RAG and memory retrieval without allowing lower-trust memories to displace
  authoritative document evidence silently.
- [x] Add query and hop budgets to prevent uncontrolled associative recall loops.

### Privacy and remote-processing controls

- [x] Make remote memory processing opt-in per tenant and sensitivity class.
- [x] Add local-only memory classifications and a local-model fallback.
- [x] Redact configured secrets and sensitive fields before remote processing.
- [x] Record exactly which evidence was transmitted to which provider and under which policy.
- [x] Support provider retention, residency, batching, timeout, and maximum-content policies.
- [x] Prevent credentials, hidden policy, unrelated tenant data, and unauthorized memories from
  entering prompts or model-visible tool output.

### Memory evaluation and acceptance criteria

- [x] Build long-running conversation and task scenarios covering correct recall, omission,
  correction, contradiction, temporal updates, procedural learning, and prospective reminders.
- [x] Measure useful recall, missed recall, intrusive recall, stale recall, harmful recall,
  cross-tenant leakage, deletion correctness, latency, token overhead, and remote cost.
- [x] Test prompt injection attempting to create policy, credential, identity, or cross-tenant
  memories.
- [x] Verify deterministic fallback when the memory model is unavailable or produces invalid JSON.
- [x] Require memory processing to remain outside the latency-critical retrieval path unless the
