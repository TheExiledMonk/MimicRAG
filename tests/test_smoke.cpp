...
int main() {
    ...
    users.AddField(pcdb::FieldVector("country", pcdb::FieldType::kInt32));
    assert(users.Name() == "users");
    assert(users.Fields().size() == 2);
    assert(users.RowCount() == 0);
    assert(users.Append({pcdb::FieldValue::Int32(34), pcdb::FieldValue::Int32(45)}));
    assert(users.Append({pcdb::FieldValue::Int32(21), pcdb::FieldValue::Int32(45)}));
    assert(users.RowCount() == 2);
    assert(users.Fields()[0].Size() == 2);
    assert(users.Fields()[1].Size() == 2);

    pcdb::Segment segment(8, users.Fields());
    ...
    pcdb::SegmentReader bad_reader(bad_path);
    ...
    pcdb::SegmentReader bad_version_reader(bad_version_path);
